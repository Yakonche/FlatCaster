...
class Entity:
    ...
    def _draw_flagellum(self, surface, start_pos, angle, length, color, wave_speed=0.2, amplitude=5):
        ...
        for i in range(1, segments + 1):
            ...
            # Onde sinusoïdale qui se propage le long de la queue
            wave = math.sin(time - i * 0.5) * (amplitude * (i / segments))

            # Rotation du point d'ondulation selon l'angle de l'entité
            perp_angle = angle + math.pi / 2

            # Position de base le long de l'angle inverse (derrière l'entité)
            base_x = start_pos[0] - math.cos(angle) * dist
            base_y = start_pos[1] - math.sin(angle) * dist

            # Ajout de l'ondulation
    ...
    def _draw_amoeba_body(self, surface, center, radius, color, time):
        """Dessine un corps qui se déforme doucement"""
        points = []
        num_points = 12
        for i in range(num_points):
            theta = (i / num_points) * math.pi * 2
            # Variation du rayon basée sur le temps et l'angle
    ...
    def _draw_virus_spikes(self, surface, center, radius, color, angle):
        """Dessine un corps avec des pointes rigides (style virus)"""
        points = []
        num_spikes = 8
        time = pygame.time.get_ticks() * 0.002

        # Rotation lente du corps entier
        ...
        pygame.draw.polygon(surface, color, points)
        pygame.draw.polygon(surface, (255, 255, 255), points, 1)  # Outline

    def _draw_cilia(self, surface, center, radius, color):
        """Dessine des petits poils autour du corps"""
        num_cilia = 16
    ...
    def draw_2d(self, surface, offset_x, offset_y, zoom):
        ...
        # --- SELECTION DU STYLE BIOLOGIQUE ---

        if self.shape_type == "flagellate":
            # Style "Spermatozoïde" : Petite tête, longue queue nerveuse
            # Queue
            ...
            # Style "Méduse" : Demi-cercle et multiples tentacules
            # 3 Tentacules
            ...
            for i in range(steps + 1):
                angle_offset = (i / steps) * math.pi - math.pi / 2  # -90 à +90 degrés relatif
            ...
            # Style "Coquille" : Rond et dur, s'ouvre légèrement si actif
            active_pulse = 0
            ...
            # Style par défaut : "Cilié" (Bactérie poilue)
